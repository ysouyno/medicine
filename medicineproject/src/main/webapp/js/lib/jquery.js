(function() {
	 ...
	 * 
	 * $Date: 2007-12-20 14:36:56 +0100 (Don, 20 Dez 2007) $ $Rev: 4251 $
	 */
	...
	jQuery.fn = jQuery.prototype = {
		init : function(selector, context) {
			...
			selector.constructor == Array
					...
					// HANDLE: $(arraylike)
					// Watch for when an array-like object, contains DOM nodes,
					// is passed in as the selector
	...
	jQuery
			.extend({
				...
				grep : function(elems, callback, inv) {
					// If a string is passed in for the function, make a
					// function
					// for it (a handy shortcut)
					if (typeof callback == "string")
						callback = eval("false||function(a,i){return "
								+ callback + "}");
	...
	jQuery.each([ "Height", "Width" ], function(i, name) {
		...
		jQuery.fn[type] = function(size) {
			...
			Math.max(Math.max(document.body["scroll" + name],
					document.documentElement["scroll" + name]), Math.max(
			...
			.extend({
						...
						if (t && !foundToken) {
							...
							if (!t.indexOf(",")) {
								...
								if (m[1] == "#" && elem && elem.getElementById
									...
									// avoid selecting an element with the name
									// of 'id' inside a form
				...
				filter : function(t, r, not) {
					...
					while (t && t != last) {
						...
						// We can get a big speed boost by filtering by class
						// here
						else if (m[1] == ".")
							...
							// We can get a speed boost by handling nth-child
							// here
