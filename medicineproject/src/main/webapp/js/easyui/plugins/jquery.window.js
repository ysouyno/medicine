...
(function($) {
	...
	function _9(_a) {
		...
		if (_b.options.modal == true) {
			_b.mask = $("<div class=\"window-mask\"></div>").insertAfter(
					_b.window);
			_b.mask.css({
		...
		if (_b.options.shadow == true) {
			_b.shadow = $("<div class=\"window-shadow\"></div>").insertAfter(
					_b.window);
			_b.shadow.css({
				display : "none"
			});
