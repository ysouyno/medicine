	...
	$.formValidator = {
		...
		bindSubmit : function(settings) {
			if (settings.ajaxCountValid > 0
					&& settings.submitAfterAjaxPrompt != "") {
		...
		resetTipState : function(validatorGroup) {
			...
			$.each(initConfig.validObjects, function() {
				...
				$.formValidator
						.setTipState(elem, passed ? "onCorrect" : "onShow",
								passed ? setting.onCorrect : setting.onShow);
			});
		},
			...
			$
					.ajax({
						...
						success : function(data, textStatus, jqXHR) {
							...
							$.formValidator.dealAjaxRequestCount(
									validatorGroup, -1);
							// 根据业务判断设置显示信息
							...
							$.formValidator
									.setTipState(elem, ls_status, ls_msg);
							// 提交的时候触发了ajax校验，等ajax校验完成，无条件重新校验
							if (returnObj.initConfig.status == "sumbitingWithAjax"
									&& returnObj.initConfig.ajaxCountSubmit == 0) {
								if (initConfig.formID != "") {
									$('#' + initConfig.formID)
											.trigger('submit');
								} else if (initConfig.formID != "") {
									$('#' + initConfig.submitButtonID).trigger(
											'click');
								}
							}
						},
	...
	$.fn.ajaxValidator = function(controlOptions) {
		...
		return this.each(function() {
			...
			if ((ajax + ",").indexOf("#" + this.id + ",") == -1) {
				initConfig.ajaxObjects = ajax + (ajax != "" ? ",#" : "#")
						+ this.id;
			}
