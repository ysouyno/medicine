...
function AjaxOption() {
	...
	// post方式
	this._initPostRequest = function(async, url, requestDataType,
			returnDataType, requestData) {
		this.asyncFlg = async;
	...
	// post方式提交，取得HTML资源
	this._initPostRequestReturnHtml = function(dataType, requestdata, async,
			isLoading, url) {
		this.requestType = 'POST';
	...
	// post方式提交，取得JSON资源
	this._initPostRequestReturnJson = function(dataType, requestdata, async,
			isLoading, url) {
		this.requestType = 'POST';
		this.requestDataType = dataType;
		this.requestData = requestdata;
...
function _ajaxGetRequest(ajaxOption, htmlElementId, callBackMethod) {
	$.ajax({
		...
		beforeSend : function(XMLHttpRequest) {
			...
			/*
			 * if(ajaxOption.isLoadingFlg == true){ showLoadingNew(); }
			 */
		},
...
function _ajaxPostRequest(ajaxOption, htmlElementId, callBackMethod) {
	$.ajax({
		...
		beforeSend : function(XMLHttpRequest) {
			...
			// 追加此请求头后，可以在一个单独的连接上进行多个请求。
			XMLHttpRequest.setRequestHeader('Connection', 'keep-alive');
			// $("body").append('<div id="load" style="left:400px;
			// position:absolute; top:250px"><img
			// src="'+BaseUrl+'images/loading2.gif" /></div>');
		},
		success : function(returnData, textStatus) {
			// currentlyAjaxRequestCount -= 1;
			if (typeof callBackMethod == 'function') {// 如何回写页面的方法参数，传递得是一个function的话，只把请求回来的数据传给这个function，共通不会自动调用回写画面方法。
