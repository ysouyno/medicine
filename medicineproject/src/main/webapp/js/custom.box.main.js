...
function getDictinfoIdlist(typecode, id) {
	...
	/*
	 * if(selectedvalue == ''){ selectobj.options[selectobj.length-1].selected =
	 * true; }
	 */
		...
		callback : function(data) {
			if (data) {
				for (var m = 0; m < data.length; m++) {
					...
					alert(info + "   " + value);
					selectobj.options[selectobj.length] = new Option(info,
							value);
					/*
...
/**
 * typecode为数据字典的typecode id：select下拉框的id
 * 
 */
...
messagewindow.prototype.createWindow = function() {
	...
	active.innerHTML = "<div id="
			+ this.windowid
			+ " class=\"easyui-window\" modal='true' closed='true'  iconCls=\"icon-save\" >"
			+ this.html + "</div>";
...
// 系统提交提示窗口显示
function submit_alert_show(iconType, messageCode, message, messagelist, width,
		height) {
...
// ***********ztree封装类*****************
function createSyncTree(treeareaid, treeid, textid, valid, setting, zNodes,
		onclickLevel, selectevent, startNodes) {
	...
	if (startNodes) {
		...
		for (var vi = 0; vi < startNodes_l.length - 1; vi++) {
			var nodetm = this.zTree
					.getNodeByParam("id", startNodes_l[vi], null); // 根据属性查找出对应的结点对象
...
createSyncTree.prototype.onBodyDown = function() {
	...
	var onBodyDown = function(event) {
		if (!(event.target.id == treeareaid || $(event.target).parents(
				"#" + treeareaid).length > 0)) {
